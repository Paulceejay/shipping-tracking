...
const CompanyIcons = () => {
    return (
      <>
        {icons.map(icon => {
          return (
            <div key={icon.src} className="bg-lightBlue lg:m-2 m-2 flex justify-center items-center flex-col  py-4 md:px-14 px-7">
              <Image
                src={icon.src}
                alt=""
                width={icon.width}
                height={icon.height}
              />
              <p className="text-white mt-4 font-light lg:text-base text-sm">{icon.name}</p>
            </div>
          );
        })}
